...
import { ShieldCheck, Check, AlertCircle, ChevronRight, HelpCircle } from 'lucide-react';
      ...
      <div className="flex items-center justify-between mb-2 pb-2 border-b border-dashed border-[#4a3427]/30">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="flex items-center gap-1">
              <span className="font-serif font-black text-xs tracking-wide text-[#dfab6c] uppercase">{locale === 'zh' ? 'MetaMask 小狐狸' : 'MetaMask Wallet'}</span>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <span className="text-[8.5px] text-[#8e7564] font-mono tracking-wider uppercase font-bold">
                {locale === 'en' ? 'Vault Shield Enabled' : '多签防护正常'}
              </span>
            </div>
          </div>
        </div>
        ...
        {pendingApprovals.length > 0 && (
          <span className="bg-[#c3432b]/10 text-[#dfab6c] border border-[#c3432b]/20 text-[8.5px] font-mono font-bold px-1.5 py-0.5 rounded animate-bounce shrink-0">
            {locale === 'zh' ? `${pendingApprovals.length} 条待签共签` : `${pendingApprovals.length} PENDING GATES`}
          </span>
        )}
      </div>
